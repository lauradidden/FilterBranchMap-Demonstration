...
def build_evaluation_log(log: pd.DataFrame):
    ...
    source = log.copy().sort_values(
        [PM_CASE_ID, PM_TIMESTAMP], kind="stable"
...
def add_variant_rank(source: pd.DataFrame) -> pd.DataFrame:
    ...
    counts = case_variants.value_counts().rename_axis("variant").reset_index(name="case_count")
...
def register_variant_stack(log_view, source: pd.DataFrame) -> dict[str, Any]:
    ...
    _name_pair(first_four, after_four, "rs_FirstFourVariants", "complement_VariantsAfterFour")
    first_two, three_and_four = log_view.evaluate_query(
        "rs_FirstTwoVariants", first_four, query_first_two_variants
    )
    _name_pair(first_two, three_and_four, "rs_FirstTwoVariants", "complement_VariantsThreeAndFour")
    return {
...
def register_without_payment(log_view, first_four: pd.DataFrame) -> dict[str, Any]:
    ...
    _name_pair(without_payment, with_payment, "rs_WithoutPayment", "complement_WithPayment")
    return {
...
def add_reference_variant_rank(source: pd.DataFrame) -> pd.DataFrame:
    ...
    credit_ids = set(source.loc[source[PM_ACTIVITY] == "Send for Credit Collection", PM_CASE_ID])
    ...
    case_variants = candidates.sort_values(
        [PM_CASE_ID, PM_TIMESTAMP], kind="stable"
    ).groupby(PM_CASE_ID)[PM_ACTIVITY].apply(tuple)
    counts = case_variants.value_counts().rename_axis("reference_variant").reset_index(name="case_count")
    counts["reference_variant_rank"] = counts.index + 1
    lookup = dict(zip(counts["reference_variant"], counts["reference_variant_rank"]))
    source["reference_variant_rank"] = source[PM_CASE_ID].map(case_variants.map(lookup))
    source.name = "FullLog"
    return counts
...
def first_later_activity_gap_days(event_log: pd.DataFrame, first_activity: str, second_activity: str) -> pd.DataFrame:
    ...
    for case_id, case in event_log.groupby(PM_CASE_ID):
        ...
        later = case.loc[
            (case[PM_ACTIVITY] == second_activity) & (case[PM_TIMESTAMP] > first_time), PM_TIMESTAMP
        ]
        if later.empty:
            continue
...
def add_delayed_notification_attributes(source: pd.DataFrame, conformance_df: pd.DataFrame) -> None:
    ...
    gap_by_case = gaps.drop_duplicates(PM_CASE_ID).set_index(PM_CASE_ID)["days_between"]
...
def register_delayed_notification_stack(log_view, source: pd.DataFrame, conformance_df: pd.DataFrame) -> dict[str, Any]:
...
def _register_appeal_stack(log_view, source, appeal_query, dismissal_query, prefix):
    appeals_name, dismissed_name = f"rs_{prefix}Appeals", f"rs_{prefix}Dismissed"
    appeals, no_appeal = log_view.evaluate_query(appeals_name, source, appeal_query)
    _name_pair(appeals, no_appeal, appeals_name, f"complement_Without{prefix}Appeal")
    dismissed, not_dismissed = log_view.evaluate_query(dismissed_name, appeals, dismissal_query)
    _name_pair(dismissed, not_dismissed, dismissed_name, f"complement_{prefix}AppealsNotDismissed")
    return {
        "appeals": appeals,
        "without_appeal": no_appeal,
        "dismissed": dismissed,
        "not_dismissed": not_dismissed,
        "queries": [appeal_query, dismissal_query],
        "result_set_name": dismissed_name,
    }
...
def register_prefecture_stack(log_view, source: pd.DataFrame) -> dict[str, Any]:
    ...
    return _register_appeal_stack(
        log_view, source, query_notification_to_prefecture_appeal, query_prefecture_dismissed, "Prefecture"
    )
...
def register_judge_stack(log_view, source: pd.DataFrame) -> dict[str, Any]:
    ...
    return _register_appeal_stack(
        log_view, source, query_notification_to_judge_appeal, query_judge_dismissed, "Judge"
    )
...
def evaluate_dismissal_comparison(log_view, source: pd.DataFrame) -> dict[str, Any]:
    """Register and compare credit-collection dismissal categories."""
    classification_summary = add_nil_dismissal_status(source)

    with_credit, without_credit = log_view.evaluate_query(
        "rs_WithCreditCollection", source, query_with_credit_collection
    )
    _name_pair(with_credit, without_credit, "rs_WithCreditCollection", "complement_WithoutCreditCollection")

    only_nil, not_only_nil = log_view.evaluate_query(
        "rs_CreditCollectionOnlyNIL", with_credit, query_dismissal_is_only_nil
    )
    _name_pair(only_nil, not_only_nil, "rs_CreditCollectionOnlyNIL", "complement_CreditCollectionNotOnlyNIL")

    contains_g, not_g = log_view.evaluate_query(
        "rs_CreditCollectionDismissalG", with_credit, query_dismissal_is_g
    )
    _name_pair(contains_g, not_g, "rs_CreditCollectionDismissalG", "complement_CreditCollectionNotG")

    contains_prefecture, not_prefecture = log_view.evaluate_query(
        "rs_CreditCollectionDismissalPrefecture", with_credit, query_dismissal_is_prefecture
    )
    _name_pair(
        contains_prefecture, not_prefecture,
        "rs_CreditCollectionDismissalPrefecture",
        "complement_CreditCollectionNotPrefecture",
    )
    ...
    summary = pd.DataFrame([
        ...
        {"category": 'Credit collection and contains dismissal = "#"', "cases": contains_prefecture[PM_CASE_ID].nunique()},
    ])

    return {
        "classification_summary": classification_summary, "summary": summary,
        "with_credit_collection": with_credit, "without_credit_collection": without_credit,
        "only_nil": only_nil, "not_only_nil": not_only_nil,
        "contains_g": contains_g, "not_g": not_g,
        "contains_prefecture": contains_prefecture, "not_prefecture": not_prefecture,
        "queries": [
            query_with_credit_collection, query_dismissal_is_only_nil,
            query_dismissal_is_g, query_dismissal_is_prefecture,
        ],
        "result_set_name": None,
    }

def _payment_case_values(log: pd.DataFrame) -> pd.DataFrame:
    required = {PM_CASE_ID, PM_TIMESTAMP, "amount", "expense", "totalPaymentAmount"}
    missing = required - set(log.columns)
    if missing:
        raise KeyError(f"Missing required columns: {sorted(missing)}")

    working = log[list(required)].copy()
    working[PM_TIMESTAMP] = pd.to_datetime(working[PM_TIMESTAMP], errors="coerce", utc=True)

    for column in ["amount", "expense", "totalPaymentAmount"]:
        working[column] = pd.to_numeric(working[column], errors="coerce")

    working["_original_order"] = range(len(working))
    working = working.sort_values([PM_CASE_ID, PM_TIMESTAMP, "_original_order"], kind="stable")
    ...
    columns = ["updated_fine_amount", "expense", "final_total_payment"]
    values[columns] = values[columns].fillna(0.0)
    return values

def calculate_outstanding_payment(log: pd.DataFrame) -> pd.Series:
    return _payment_balance(log).clip(lower=0).rename("metric_value")


def _payment_balance(log: pd.DataFrame) -> pd.Series:
    values = _payment_case_values(log)
    return values["updated_fine_amount"] + values["expense"] - values["final_total_payment"]


def calculate_paid_percentage(log: pd.DataFrame) -> pd.Series:
    """Return 100 for fully paid cases and 0 otherwise."""
    return ((_payment_balance(log) <= 0.01).astype(float) * 100).rename("metric_value")
...
# Backwards-compatible name without duplicating the implementation.
time_between_activities = first_later_activity_gap_days


def cases_with_followed_by_relation(event_log: pd.DataFrame, first_activity: str, second_activity: str) -> set:
